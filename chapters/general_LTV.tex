...
L_H(t,f)\mathop {\rm{ = }}\limits^{\Delta} \int_{ - \infty }^\infty  {\int_{ - \infty }^\infty  S_H(\tau,\nu)e^{-j2\pi(f\tau-t\nu)}d\tau d\nu }
\end{equation}
...
\begin{equation} \label{eq2_15}
x[n]= \frac{1}{\sqrt{N}}\sum\limits_{k = 0}^{N-1} X[k]e^{j2\pi nk/N}
\end{equation}

假设信道最大多径时延为$\tau_{max}$，则要加入长度为$L_{cp} > \tau_{max}$的保护间隔(GI)，避免符号间干扰。保护间隔一般取循环前缀(CP) 的形式，即将OFDM 符号后面的$L_{cp}$个符号复制到最前面，形成$L_{cp}+N$ 的新序列。

\begin{table}[!htbp]
\renewcommand{\arraystretch}{1.5}
\zihao{6}\caption{CP示意图}
\begin{tabular}{|c|c|c|}
\hline
$x[N-L_{cp}],x[N-L_{cp}+1],...,x[N-1]$ & $x[0],x[1],...,x[N-L_{cp}-1]$ & $x[N-L_{cp}],x[N-L_{cp}+1],...,x[N-1]$ \\
\hline
长度为$L_{cp}$的循环前缀 & \multicolumn{2}{c|}{长度为 $N$ 的原始序列} \\
\hline
\end{tabular}
\end{table}
...
\begin{aligned}
y[n] & =\sum\limits_{m = 0}^{M-1} h[n,m]x[n-m] +w[n] \\
& = \sum\limits_{m = 0}^{M-1} h[n,m]\frac{1}{\sqrt{N}}\sum\limits_{k = 0}^{N-1} X[k]e^{j2\pi nk/N} + w[n]
\end{aligned}
\end{equation}

这里，我们只考虑离散形式，即图\ref{p_ofdm}中虚线部分等效为离散信道,最大多径时延$\tau_{max}$对应的采样符号为$M$。 式中$h[n,m]$ 对应LTV信道模型中的时变脉冲响应,表示时延为$m$时第$n$ 个符号处的信道增益，$w[n]$ 是加性高斯白噪声。
...
式中$\mathbf{F}$为$N$维傅里叶变换矩阵，$\mathbf{h}$为时域信道矩阵的圆卷积形式
\begin{equation} \label{eq2_15}
{\bf{h}} = \left[ {\begin{array}{*{20}{c}}
{h\left[ {0,0} \right]}&0& \cdots &{h\left[ {N - M,M - 1} \right]}& \cdots &{h\left[ {N - 1,1} \right]}\\
{h\left[ {0,1} \right]}& \ddots &0& \cdots & \cdots &{h\left[ {N - 1,2} \right]}\\
 \vdots & \ddots & \ddots & \ddots & \ddots & \vdots \\
{h\left[ {0,M - 1} \right]}& \ddots & \ddots & \ddots & \ddots &0\\
0& \ddots & \ddots & \ddots & \ddots &0\\
0& \cdots &{h\left[ {N - M - 1,M - 1} \right]}& \cdots & \cdots &{h\left[ {N - 1,0} \right]}
\end{array}} \right]
\end{equation}
...
对于静态信道，$\mathbf{h}$是一个Toeplitz矩阵，即每列应该是第一列的移位，对应的频域矩阵$\mathbf{H=FhF^{H}}$是一个对角矩阵。对于时变信道，矩阵$\mathbf{h}$ 各列数据不再相等，频域矩阵$\mathbf{H}$也不再是对角阵。因此对时变信道的估计，在时域，待估计参数从$M$ 个增加到$MN$ 个；在频域，待估计参数由静态信道的$N$个扩展为$N^2$ 个。
...
\section{使用压缩感知进行双选信道信道估计} \label{sec2_5_1}
