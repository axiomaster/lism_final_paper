...
\item $\mathcal{P}$ \hspace{12.5em} 信道多径径数
\item $P$ \hspace{12.5em} 导频数量
\item $v$ \hspace{12.5em} 移动速度
\item $\nu$ \hspace{12.5em} 多普勒频移
\item $K$ \hspace{12.5em} 子载波个数
\item $L$ \hspace{12.5em} OFDM符号数
\item $L_{cp}$ \hspace{12.5em} 循环前缀长度
\item \ldots
\end{symbollist}
